...
class DeviceOdometer():
    ...
    def get_odoometer(self, device_ids):
        ...
        if status_datas:
            values = {
                status_data[0].get("device").get("id"): {"odometer_2": status_data[0].get("data"), "datetime_2": status_data[0].get("dateTime").strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        try:
            ...
            if devices:
                ...
                if datas:
                    ...
                    return "from script_pos : " + str(datas)
            else:
                return "from script_pos : No order to treat for the moment"
