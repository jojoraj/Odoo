odoo.define('evtc_lmfs_map_bo.MapRenderer', function (require) {
    "use strict"
    ...
    class MapRender extends WebMapRenderer {
        async willStart() {
            ...
            if (this.props.arch.attrs.res_partner === 'driver_id') {
                [this._carTopviewSVG,this._carTopLmfs,this._carTopGeotab] = await Promise.all([
                    this.env.services.httpRequest('evtc_lmfs_map_bo/static/src/img/car_topview.svg', p, 'text'),
                    this.env.services.httpRequest('evtc_lmfs_map_bo/static/src/img/Phone2.svg', p, 'text'),
                    this.env.services.httpRequest('evtc_lmfs_map_bo/static/src/img/router2.svg', p, 'text'),
                ]);
            } else {
        ...
        _addMarkers() {
            ...
            for (const markerInfo of Object.values(markersInfo)) {
                ...
                if(markerInfo.record.geolocalization_type === false) pinSVG = this._carTopviewSVG
                ...
                var color = ""
                if(markerInfo.record.vehicle_mo_state === "AVAILABLE"){
                    color = "#ADFF2F"
                }else{
                    color = "#FF0000"
                }
